...
const EnhancedFooter = () => {
  ...
  return (
    <footer className="relative bg-gradient-dark border-t border-primary/20 mt-20 overflow-hidden">
      {/* Newsletter section */}
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl cyber-glass border border-primary/20 bg-transparent text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-colors"
            />
            ...
            <div className="flex space-x-4">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a
                  key={label}
                  href={href}
                  className="p-3 rounded-lg cyber-glass border border-primary/20 text-muted-foreground hover:text-primary hover:border-primary/50 hover:glow-primary transition-all duration-300 group"
                  aria-label={label}
                >
            ...
            <ul className="space-y-3">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <Link
                    to={link.path}
                    className="text-muted-foreground hover:text-primary transition-colors duration-300 relative group text-sm"
                  >
                    {link.name}
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-96 h-96 bg-primary/3 rounded-full blur-3xl animate-gentle-glow" style={{ animationDelay: '4s' }} />
      </div>
    </footer>
  );
};
